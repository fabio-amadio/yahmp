...
def push_moving_envs_by_setting_velocity(
    ...
    """Push only the envs whose velocity command is non-negligible.

    Standing-commanded envs are left unperturbed. Pushing a robot that is asked
    to hold still teaches it to recruit arms/torso for reactive balance even at
    rest, which produces the standing jitter (trembling torso, fidgeting arms)
    we want to avoid. Locomotion-commanded envs are still pushed, preserving the
    perturbation robustness -- and the arm swing it elicits -- during walking and
    running. Command magnitude (|lin| + |ang|) is used as the moving/standing
    gate, so this also spares envs whose sampled command happens to be near zero,
    not just the explicitly standing fraction.
    """
    command = env.command_manager.get_command(command_name)
    assert command is not None
    speed = torch.norm(command[env_ids, :2], dim=1) + torch.abs(command[env_ids, 2])
    moving_env_ids = env_ids[speed > command_threshold]
    if moving_env_ids.numel() == 0:
        return
    push_by_setting_velocity(
        env, moving_env_ids, velocity_range=velocity_range, asset_cfg=asset_cfg
    )


def push_moving_envs_by_robot_speed(
    env: ManagerBasedRlEnv,
    env_ids: torch.Tensor,
    velocity_range: dict[str, tuple[float, float]],
    ...
    lin_speed = torch.norm(asset.data.root_link_lin_vel_b[env_ids, :2], dim=1)
    ang_speed = torch.abs(asset.data.root_link_ang_vel_b[env_ids, 2])
    speed = lin_speed + ang_speed
    moving_env_ids = env_ids[speed > speed_threshold]
    if moving_env_ids.numel() == 0:
        return
    push_by_setting_velocity(
        env, moving_env_ids, velocity_range=velocity_range, asset_cfg=asset_cfg
...
class apply_torque_limited_body_force:
    ...
    def __init__(self, cfg, env: ManagerBasedRlEnv):
        ...
        if isinstance(body_ids, list):
            self._body_ids = torch.tensor(
                body_ids, device=self._device, dtype=torch.long
            )
        ...
        if len(self._body_ids) == 0:
            raise ValueError(
                "apply_torque_limited_body_force requires at least one body."
            )
        ...
        self._debug_force_vis_scale = float(
            cfg.params.get("debug_force_vis_scale", 0.015)
        )
        ...
        self._debug_force_vis_width = float(
            cfg.params.get("debug_force_vis_width", 0.01)
        )
        ...
        self._jacobian_body_ids_wp = wp.from_torch(
            self._jacobian_body_ids, dtype=wp.int32
        )
    ...
    def __call__(
        ...
        if len(active_ids) > 0:
            ...
            remaining = (
                self._active_duration_total[active_ids]
                - self._active_elapsed_time[active_ids]
            )
    ...
    def get_episode_force_magnitude_stats(
        ...
        if bool(active_mask.any().item()):
            magnitude_mean = (
                self._episode_force_mag_sum[env_ids].sum() / total_sample_count
            )
    ...
    def debug_vis(self, visualizer: DebugVisualizer) -> None:
        ...
        for env_id in env_indices:
            ...
            offset_w = quat_apply(body_quat_w.unsqueeze(0), offset_local.unsqueeze(0))[
                0
            ]
    ...
    def _compute_force_ramp_scale(self, env_ids: torch.Tensor) -> torch.Tensor:
        ...
        phase = (
            (self._active_elapsed_time[env_ids] + 0.5 * self._step_dt) / total
        ).clamp(0.0, 1.0)
    ...
    def _compute_baseline_torque(self, env_ids: torch.Tensor) -> torch.Tensor:
        ...
        effort_target = self._entity_data.joint_effort_target[env_ids][
            :, self._joint_ids
        ]
    ...
    def _sample_force_vector(
        ...
        sign = torch.where(
            (pos_capacity > self._eps) & ~both, torch.ones_like(sign), sign
        )
        sign = torch.where(
            (neg_capacity > self._eps) & ~both, -torch.ones_like(sign), sign
        )
    ...
    def _max_scale_for_direction(
        ...
        scale = torch.where(
            baseline_feasible & has_effect, scale, torch.zeros_like(scale)
        )
        return scale
    ...
    def _sample_application_points(
        ...
        if self._randomize_body:
            ...
            body_slots = torch.zeros(
                len(env_ids), device=self._device, dtype=torch.long
            )

        row_ids = torch.arange(len(env_ids), device=self._device)
        body_link_pos_all = self._entity_data.body_link_pos_w[env_ids][
            :, self._body_ids
        ]
        body_link_quat_all = self._entity_data.body_link_quat_w[env_ids][
            :, self._body_ids
        ]
