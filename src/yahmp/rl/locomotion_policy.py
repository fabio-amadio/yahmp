...
class YahmpLocomotionActorModel(MLPModel):
  ...
  def _copy_normalizer_proprio_history(
    ...
    hist_first_weight = imitation_state_dict.get("history_encoder.conv.0.weight")
    if hist_first_weight is None:
      return
    imit_step_obs_dim = int(hist_first_weight.shape[1])
    imit_motion_obs_dim = imit_step_obs_dim - self.proprio_obs_dim
