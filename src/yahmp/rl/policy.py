...
class YahmpEncoderDecoderActorModel(MLPModel):
  """Actor with an encoder-decoder MLP and state injection.

  - history_encoder: 1D-conv over history -> history_latent
  - encoder: MLP mapping concat(current_motion, proprio, history_latent) -> z
  - decoder (= self.mlp): MLP mapping concat(z, proprio, history_latent) -> action
    Proprio and history_latent are re-injected into the decoder (state injection).
  """
  ...
  def __init__(
    ...
    history_conv_strides: tuple[int, ...] | list[int] = (2, 1),
    latent_dim: int = 128,
    encoder_hidden_dims: tuple[int, ...] | list[int] = (512, 512, 256, 128),
    layer_norm: bool = True,
  ) -> None:
    self.current_motion_obs_dim = int(current_motion_obs_dim)
    self.proprio_obs_dim = int(proprio_obs_dim)
    ...
    self.encoder_hidden_dims = tuple(int(v) for v in encoder_hidden_dims)
    self.layer_norm = bool(layer_norm)
    ...
    if len(self.encoder_hidden_dims) == 0:
      raise ValueError("`encoder_hidden_dims` must contain at least one value.")

    self.current_obs_dim = self.current_motion_obs_dim + self.proprio_obs_dim
    # History stacks proprio-only per step (motion ref enters only via current).
    self.history_obs_dim = self.proprio_obs_dim * self.history_steps
    ...
    if self.obs_dim != expected_obs_dim:
      raise ValueError(
        "YahmpEncoderDecoderActorModel observation dimension mismatch: "
        f"got {self.obs_dim}, expected {expected_obs_dim} "
        f"({self.current_obs_dim} current + {self.history_obs_dim} history)."
      )
    ...
    encoder_input_dim = (
      self.current_motion_obs_dim + self.proprio_obs_dim + self.history_latent_dim
    )
  ...
  def _get_latent_dim(self) -> int:
    return self.latent_dim + self.proprio_obs_dim + self.history_latent_dim
  ...
  def get_latent(
    ...
    motion_obs, proprio_obs, history_obs = self._split_obs(obs_flat)
    history_latent = self.history_encoder(history_obs)
    encoder_input = torch.cat((motion_obs, proprio_obs, history_latent), dim=-1)
    z = self.encoder(encoder_input)
    return torch.cat((z, proprio_obs, history_latent), dim=-1)

  def as_onnx(self, verbose: bool = False) -> nn.Module:
    return _OnnxYahmpEncoderDecoderActorModel(self, verbose=verbose)


class YahmpEncoderDecoderCriticModel(MLPModel):
  """Critic with an encoder-decoder MLP and state injection.

  Encoder takes concat(current_motion, proprio, history_latent, privileged) -> z.
  Decoder takes concat(z, proprio, history_latent, privileged) -> value.
  """
  ...
  def __init__(
    ...
    if self.obs_dim != expected_obs_dim:
      raise ValueError(
        "YahmpEncoderDecoderCriticModel observation dimension mismatch: "
        f"got {self.obs_dim}, expected {expected_obs_dim} "
        f"({self.current_obs_dim} current + {self.history_obs_dim} history + "
        f"{self.privileged_obs_dim} privileged)."
      )
  ...
  def get_latent(
    ...
    z = self.encoder(encoder_input)
    return torch.cat((z, proprio_obs, history_latent, privileged_obs), dim=-1)
...
class _OnnxYahmpEncoderDecoderActorModel(nn.Module):
  """ONNX wrapper for YahmpEncoderDecoderActorModel."""

  is_recurrent: bool = False

  def __init__(
    self, model: YahmpEncoderDecoderActorModel, verbose: bool = False
  ) -> None:
    super().__init__()
    self.verbose = verbose
    self.obs_normalizer = copy.deepcopy(model.obs_normalizer)
  ...
  def forward(self, x: torch.Tensor) -> torch.Tensor:
    ...
    history_obs = x[:, current_end:history_end]
    history_latent = self.history_encoder(history_obs)
    z = self.encoder(torch.cat((motion_obs, proprio_obs, history_latent), dim=-1))
    out = self.mlp(torch.cat((z, proprio_obs, history_latent), dim=-1))
    return self.deterministic_output(out)
