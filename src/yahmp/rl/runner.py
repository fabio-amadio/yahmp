...
class YahmpOnPolicyRunner(MjlabOnPolicyRunner):
    ...
    def _maybe_upload_checkpoint(self, checkpoint_path: Path) -> None:
        ...
        if mode == "all":
            self.logger.save_model(
                str(checkpoint_path), self.current_learning_iteration
            )
            return
    ...
    def _configure_model_cfg(cls, env, train_cfg: dict) -> None:
        ...
        if (
            actor_class == "yahmp.rl.policy:YahmpFutureActorModel"
            and motion_dims is not None
        ):
            observation_manager = env.unwrapped.observation_manager
            obs_dims = observation_manager.group_obs_dim
            ...
            history_dim = flat_term_dims["history"]
            current_dim = (
                int(obs_dims[actor_group][0]) - int(motion_dims[0]) - history_dim
            )
        ...
        if (
            critic_class == "yahmp.rl.policy:YahmpCriticModel"
            and motion_dims is not None
        ):
            observation_manager = env.unwrapped.observation_manager
            obs_dims = observation_manager.group_obs_dim
            ...
            proprio_dim = (
                flat_term_dims["base_ang_vel"] + flat_term_dims["projected_gravity"]
            )
        ...
        if (
            ...
            history_step_dim = flat_term_dims["policy_history"] // max(
                history_length, 1
            )
            if history_dim % history_step_dim != 0:
                raise ValueError(
        ...
        if (
            actor_class
            == "yahmp.rl.student_teacher_policy:YahmpStudentTeacherActorModel"
    ...
    def _configure_locomotion_model_cfg(
        ...
        proprio_dim = int(obs_dims[actor_group][0]) - goal_dim - history_dim
        history_step_dim = proprio_dim
        if history_dim % history_step_dim != 0:
            raise ValueError(
        ...
        critic_group = YahmpOnPolicyRunner._obs_group_name(
            train_cfg, "critic", "critic"
        )
        critic_terms = observation_manager.active_terms[critic_group]
        critic_term_dims = observation_manager._group_obs_term_dim[critic_group]
        flat_critic_dims = {
        ...
        critic_history_step_dim = proprio_critic_dim
    ...
    def save(self, path: str, infos=None) -> None:
        ...
        try:
            ...
            attach_onnx_metadata(
                self.env.unwrapped, run_name, str(policy_path / filename)
            )
            if self.logger.logger_type in ["wandb"] and self.cfg["upload_model"]:
                wandb.save(
                    str(policy_path / filename),
                    base_path=os.path.dirname(str(policy_path)),
                )
...
class YahmpLocomotionOnPolicyRunner(YahmpOnPolicyRunner):
    ...
    @staticmethod
    def _extract_imitation_state_dict(
        loaded: dict[str, Any],
...
class YahmpStudentOnPolicyRunner(YahmpOnPolicyRunner):
    ...
    def _teacher_checkpoint_path(self) -> Path | None:
        ...
        if checkpoint_file:
            ...
            if not checkpoint_path.exists():
                raise FileNotFoundError(
                    f"Teacher checkpoint file not found: {checkpoint_path}"
                )
        ...
        if self._yahmp_log_dir is None:
            raise ValueError(
                "Cannot resolve teacher W&B checkpoint without a log directory."
            )
        log_root_path = self._yahmp_log_dir.parent
        checkpoint_path, was_cached = get_wandb_checkpoint_path(
            log_root_path,
